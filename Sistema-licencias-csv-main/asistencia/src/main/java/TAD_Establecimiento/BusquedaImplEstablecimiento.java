...
public class BusquedaImplEstablecimiento implements BusquedaBinariaEstablecimiento {
    ...
    private NodoEstablecimiento getMiddle(NodoEstablecimiento start, int length) { //Para obtener el nodo que subdivide a la mitad
        NodoEstablecimiento actual = start;
    ...
    public ListaEnlazadaImplEstablecimiento buscarPorRuc(String Ruc) {
        ...
        int length = length();
        NodoEstablecimiento inicio = cabeza;
    ...
    public ListaEnlazadaImplEstablecimiento buscarPorArea(Double Area) {
        ...
        while (length > 0) {
            NodoEstablecimiento medio = getMiddle(inicio, length);
            double areaEstab = medio.getEstablecimiento().getArea_Estab();
            int comparador = Double.compare(areaEstab, Area);
            if (comparador == 0) {
                resultado.insertar(medio.getEstablecimiento());
                break;
            } else if (comparador < 0) {
